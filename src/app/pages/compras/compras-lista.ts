  ...
  template: `
    <div class="p-6 space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <tbody class="divide-y divide-slate-100">
            @for (c of compras(); track c.id) {
              <tr class="hover:bg-slate-50">
                ...
                <td class="p-3 text-slate-500">{{ c.fecha }}</td>
                <td class="p-3">
        ...
        @if (compras().length === 0) {
          <div class="p-8 text-center text-slate-400">No hay compras registradas</div>
        }
      </div>
    </div>
  `,
...
export class ComprasListaComponent implements OnInit {
  ...
  ngOnInit() { this.tauri.listCompras().then(c => this.compras.set(c)); }
}
